<template>
  <!-- menu with submenu -->
  <li
    v-if="menu.children && checkPerms(menu.permission)"
    class="has-sub"
    v-bind:class="{ 'active': subIsActive(menu.path) }"
  >
  ...
  <router-link
    v-else-if="checkPerms(menu.permission)"
    v-bind:to="menu.path"
    active-class="active"
    tag="li"
...
export default {
  ...
  methods: {
    ...
    checkPerms(perm) {
      if (this.$isString(perm)) {
        return this.$permission(this.user.all_permissions, perm);
      } else if (Array.isArray(perm)) {
        return this.$perms_in_array(this.user.all_permissions, perm);
      } else {
        return false;
      }
    }
